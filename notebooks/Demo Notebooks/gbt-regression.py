...
# MAGIC 
# MAGIC We begin by loading our data, which is stored in [Comma-Separated Value (CSV) format](https://en.wikipedia.org/wiki/Comma-separated_values).  For that, we use the [CSV datasource for Spark](http://spark-packages.org/package/databricks/spark-csv), which creates a [Spark DataFrame](http://spark.apache.org/docs/latest/sql-programming-guide.html) containing the dataset.  We also cache the data so that we only read it from disk once.
...
# MAGIC 
# MAGIC Check out the other example notebooks in this guide for more ideas about Pipelines, working with datasets, and more. Here are some to start with:
# MAGIC * [Full ML Workflow using Pipelines](https://docs.databricks.com/spark/latest/mllib/index.html)